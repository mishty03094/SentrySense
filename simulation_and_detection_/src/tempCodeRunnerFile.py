...
stream_df_aligned = stream_df[[col for col in df_existing.columns if col in stream_df.columns]]
for col in df_existing.columns:
    if col not in stream_df_aligned.columns:
        stream_df_aligned[col] = 0
stream_df_aligned = stream_df_aligned[df_existing.columns]
...
for idx, row in stream_df_aligned.iterrows():
    ...
    if isinstance(label, str) and label.strip().lower() == 'malicious':
